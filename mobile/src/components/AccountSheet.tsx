...
function StatusPill({ status }: { status: Order["status"] }) {
  ...
  return (
    <View className={`rounded-full border px-2.5 py-1 ${s.box}`}>
      <Text className={`text-xs font-black uppercase tracking-wide ${s.text}`}>
        {status}
      </Text>
    </View>
  );
...
function MenuRow({
  ...
  return (
    ...
    >
      <View
        className={`h-9 w-9 items-center justify-center rounded-full ${danger ? "bg-red-50" : "bg-mist"}`}
      >
      ...
      <View className="min-w-0 flex-1">
        ...
        <Text
          numberOfLines={1}
          className={`text-sm font-bold ${danger ? "text-red-600" : "text-ink"}`}
        >
          {label}
        </Text>
          ...
          <View className="border-b border-line bg-mist/60 p-4">
            <View className="flex-row items-center gap-3">
                ...
                <View className="flex-row items-center gap-2">
                  ...
                  {isAdmin && (
                    <View className="shrink-0 rounded-full bg-gold px-2 py-0.5">
                      <Text className="text-[10px] font-black uppercase tracking-wide text-ink">
                        Admin
                      </Text>
                    </View>
                  )}
              ...
              <Pressable
                onPress={onRequestClose}
                accessibilityLabel="Close account"
                className="h-8 w-8 shrink-0 items-center justify-center rounded-full bg-mist"
              >
          ...
          <View className="p-4">
            ...
            {loading ? (
              ...
              orders.slice(0, 3).map((o) => (
                <View
                  key={o.id}
                  className="mt-2 rounded-xl border border-line bg-surface px-3 py-2.5"
                >
                  ...
                  <View className="mt-1.5 flex-row items-center justify-between gap-2">
                    ...
                    <Text className="text-sm font-black text-ink">
                      NPR {o.total_npr.toLocaleString()}
                    </Text>
                  </View>
                </View>
              ))
                ...
                <View className="mt-1 flex-row items-center justify-between rounded-xl border border-red-100 bg-red-50 px-3 py-2.5">
                  ...
                  <View className="flex-row items-center gap-2">
                    <Pressable
                      ...
                      accessibilityRole="button"
                      className="rounded-full border border-line bg-card px-3.5 py-1.5"
                    >
                    ...
                    <Pressable
                      ...
                      accessibilityRole="button"
                      className="rounded-full bg-red-600 px-3.5 py-1.5"
                    >
