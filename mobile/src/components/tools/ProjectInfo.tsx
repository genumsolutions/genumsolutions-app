...
export function ProjectInfo({
  ...
  const selectedMode =
    LOCAL_CAR_MODES.find((m) => m.id === selectedModeId) ?? mode;
  ...
  return (
    <View className="rounded-2xl border border-line bg-card p-5 shadow-card">
      <Pressable
        ...
        accessibilityLabel={
          expanded ? "Collapse project info" : "Expand project info"
        }
        ...
        <View className="mt-4">
          ...
          >
            {LOCAL_CAR_MODES.map((m) => {
              ...
              return (
                ...
                >
                  <Text
                    className={`text-[11px] font-bold ${isActive ? "text-white" : "text-ink"}`}
                  >
                    {m.token}
                  </Text>
                  {!isAvailable && (
          ...
          <View className="mt-4 rounded-xl border border-line bg-surface p-4">
            ...
            {CAR_PROJECT_NAME[selectedMode.token] && (
              <InfoRow
                label="Car"
                value={CAR_PROJECT_NAME[selectedMode.token]}
              />
            )}
            ...
            <InfoRow
              label="Sensors"
              value={
                selectedMode.sensors.length
                  ? selectedMode.sensors.join(", ")
                  : "—"
              }
            />
            <InfoRow
              label="Control link"
              value={selectedMode.transport.join(", ")}
            />
            ...
            <Text className="mt-3 text-[9px] text-muted">
              Asset of {company.name}
            </Text>
          </View>
        </View>
      )}
      ...
      {expanded && !isRobocar && category && (
        <View className="mt-4">
          ...
          <View className="flex-row items-start justify-between gap-3 border-b border-line py-2 last:border-b-0">
            <Text className="shrink-0 text-xs font-bold uppercase tracking-wide text-muted">
              Capabilities
            </Text>
            <View className="min-w-0 flex-1 flex-row flex-wrap justify-end gap-1.5">
              {category.capabilities.map((cap) => (
                <Text
                  key={cap}
                  className="rounded-full bg-mist px-2 py-0.5 text-[10px] font-bold text-navy"
                >
...
function InfoRow({
  ...
  return (
    <View className="flex-row items-start justify-between gap-3 border-b border-line py-2 last:border-b-0">
      ...
      <Text
        numberOfLines={2}
        className={`min-w-0 flex-1 text-right text-xs text-ink ${mono ? "font-mono" : ""} ${strong ? "font-bold" : ""}`}
      >
        {value}
      </Text>
    </View>
