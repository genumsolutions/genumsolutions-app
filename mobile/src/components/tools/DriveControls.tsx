...
function clampServo(v: number, l: SafetyLimits) {
  return Math.max(
    l.servoCenter - l.maxSteerDeviation,
    Math.min(180, l.servoCenter + l.maxSteerDeviation, Math.round(v)),
  );
}
    ...
    <View
      ...
      className={`items-center justify-center rounded-xl ${
        active
          ...
          : enabled
            ? "border-2 border-slate-300 bg-slate-100 dark:border-white/25 dark:bg-white/8"
...
function DualDpad({
  ...
  const resolvePoint = useCallback(
    (x: number, y: number): { pad: PadId; zone: PadZone } | null => {
      ...
      const dx = lx - pw / 2;
      const dy = y - surf.h / 2;
      const threshold = Math.min(pw, surf.h) * 0.2;
      const zone: PadZone =
        Math.abs(dx) < threshold && Math.abs(dy) < threshold
          ...
          : Math.abs(dy) >= Math.abs(dx)
            ...
            : pad === "L"
              ? dx < 0
                ? "L"
                : "R" // Left pad: left=L, right=R (natural)
              : dx < 0
                ? "R"
                : "L"; // Right pad: swapped for ESP remote parity
      return { pad, zone };
    },
    [surf],
  );
  ...
  const emit = useCallback(
    (touches: Map<string, { pad: PadId; zone: PadZone }>) => {
      ...
      for (const [, cell] of touches) {
        if (cell.pad === "L" && en.L.includes(cell.zone))
          lZones.push(cell.zone);
        else if (cell.pad === "R" && en.R.includes(cell.zone))
          rZones.push(cell.zone);
      }
      ...
      if (s.is2wd1m) {
        ...
        const d: PadZone | "S" = has(lZones, "C")
          ? "C"
          : has(lZones, "F")
            ? "F"
            : has(lZones, "B")
  ...
  const publishState = useCallback(() => {
    ...
    for (const [, cell] of touchesRef.current) {
      ...
      seen.add(sig);
      const key = (cell.pad +
        (next[(cell.pad + "1") as "L1" | "R1"] ? "2" : "1")) as
        "L1" | "L2" | "R1" | "R2";
      next[key] = cell.zone;
    }
    const sig = Object.keys(next)
      .sort()
      .map((k) => `${k}:${next[k as keyof typeof next]}`)
      .join(",");
    setActiveCells((prev) => {
      const prevSig = Object.keys(prev)
        .sort()
        .map((k) => `${k}:${prev[k as keyof typeof prev]}`)
        .join(",");
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderRelease: (evt) => {
          ...
          publishState();
          emit(touchesRef.current);
          if (touchesRef.current.size === 0) setActiveCells({});
        },
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (stRef.current.is2wd1m) {
        ...
        if (hasL) {
          if (onSignedDriveRef.current)
            onSignedDriveRef.current(lastDriveRef.current);
          else
            sendDirRef.current(
              lastDriveRef.current > 0
                ? "F"
  ...
  const cellActive = (pad: PadId, zone: PadZone) =>
    Object.entries(activeCells).some(
      ([k, z]) => k.startsWith(pad) && z === zone,
    );
      ...
      <View style={{ width: cellSize * 3 + gap * 2 }}>
        ...
        <View style={{ flexDirection: "row", gap, marginBottom: gap }}>
          ...
          <View style={{ width: cellSize, height: cellSize }}>
            <DpadCell
              icon={PAD_ICONS.F}
              active={cellActive(pad, "F")}
              enabled={en[pad].includes("F")}
            />
          </View>
          <DpadGap />
        </View>
        ...
        <View style={{ flexDirection: "row", gap }}>
          ...
          <View style={{ width: cellSize, height: cellSize }}>
            <DpadCell
              icon={PAD_ICONS.B}
              active={cellActive(pad, "B")}
              enabled={en[pad].includes("B")}
            />
          </View>
          <DpadGap />
        </View>
...
function DualJoystick({
  ...
  const radius = geo ? Math.min(geo.w * 0.18, geo.h * 0.3, 85) : 0;
  const centerOf = (stick: "L" | "R") =>
    geo
      ? { cx: geo.w * (stick === "L" ? 0.25 : 0.75), cy: geo.h * 0.5 }
      : { cx: 0, cy: 0 };
  ...
  const resolveStick = (x: number, y: number): "L" | "R" => {
    ...
    const l = centerOf("L"),
      r = centerOf("R");
    return Math.hypot(x - l.cx, y - l.cy) <= Math.hypot(x - r.cx, y - r.cy)
      ? "L"
      : "R";
  };

  const applyKnob = useCallback(
    (stick: "L" | "R", dx: number, dy: number, report = true) => {
      const cap = radius || 1;
      let nx = dx,
        ny = dy;
        ...
        if (report) {
          ...
          if (navActiveRef?.current && onNavInputRef.current) {
            if (Math.abs(nX) >= Math.abs(nY)) {
              if (Math.abs(nX) > 0.35)
                onNavInputRef.current("x", nX < 0 ? -1 : 1);
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderRelease: (evt) => {
          ...
          if (!hasL) {
            applyKnob("L", 0, 0, true);
            setKnobL({ x: 0, y: 0 });
          }
          if (!hasR) {
            applyKnob("R", 0, 0, true);
            setKnobR({ x: 0, y: 0 });
          }
        },
        onPanResponderTerminate: (evt) => {
  ...
  return (
    <View
      onLayout={(e) => {
        ...
        setGeo((g) =>
          g && g.w === width && g.h === h ? g : { w: width || 1, h: h || 1 },
        );
      }}
      {...panResponder.panHandlers}
      ...
      {geo && radius > 0 && (
        <>
          {(["L", "R"] as const).map((stick) => {
            ...
            const tag =
              stick === "L" ? "DRIVE" : rightEnabled ? "STEER" : "UNUSED";
              ...
              <View key={stick}>
                <View
                  ...
                  style={{
                    ...
                    width: 60,
                    alignItems: "center",
                    top: Math.max(4, c.cy - radius - 26),
                  }}
                >
                  <Text className="text-[9px] font-black uppercase tracking-[0.22em] text-slate-400 dark:text-slate-300">
                    {tag}
                  </Text>
                </View>
                <View
...
export function DriveControls({
  ...
  const showSpeed =
    activeMode.controls.includes("drive-tank") ||
    activeMode.controls.includes("drive-2wd1m");
  const showPid = activeMode.controls.includes("pid-auto");
  const showStartStop =
    activeMode.controls.includes("start-stop") ||
    activeMode.controls.includes("tuning");
  ...
  const sendDir = useCallback(
    (d: "F" | "B" | "L" | "R" | "S") => {
      const now = Date.now();
      if (lastDirRef.current === d && now - lastDirTimeRef.current < 100)
        return;
  ...
  const handleLeftJoy = useCallback(
    (x: number, y: number) => {
      if (!canControl) return;
      ...
      const d = is2wd1m
        ? y < -0.25
          ...
          : y > 0.25
            ? "B"
            : "S"
        : Math.abs(x) < 0.25 && Math.abs(y) < 0.25
          ? "S"
  ...
  const handleRightJoy = useCallback(
    (x: number) => {
      ...
      {showSpeed && !compact && (
        <View className="mt-4 rounded-xl border border-line bg-surface p-3">
          ...
          <View className="mt-1 flex-row items-center justify-center gap-3">
            <MiniStepperBtn
              onPress={() => onSpeed(clampSpeed(speed - 5, limits))}
              disabled={!canControl}
              icon="minus"
            />
            <MiniStepperBtn
      ...
      {showPid && !compact && (
        <View className="mt-4 flex-row flex-wrap gap-3">
          {[
            ...
            <View
              key={p.key}
              className="w-[48%] rounded-xl border border-line bg-surface p-4"
            >
              <Text className="text-sm font-bold uppercase tracking-wide text-muted">
                {p.label}
              </Text>
              <Slider
                value={p.val}
                minimumValue={p.key === "off" ? -5 : 0}
                maximumValue={p.max}
                step={p.step}
                onValueChange={(v: number) => onPid(p.key, v)}
                disabled={!canControl}
                minimumTrackTintColor="#1e3a8a"
                maximumTrackTintColor="#cbd5e1"
                thumbTintColor="#1e3a8a"
