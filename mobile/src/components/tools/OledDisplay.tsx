...
export function OledDisplay({
  ...
  const body = (() => {
    if (previewMode && previewModeAvail) {
      ...
      if (previewModeAvail === "WIP")
        return { kind: "coming" as const, text: "WORK IN PROGRESS" };
    }
    if (previewComingSoon)
      return { kind: "coming" as const, text: "COMING SOON" };
    if (previewMode) {
      if (
        previewMode.token === "BT" ||
        previewMode.token === "4WD4M" ||
        previewMode.controls.includes("drive-2wd1m")
      ) {
  ...
  return (
    <View
      className={
        compact
          ? "h-full w-full flex-col rounded-xl bg-slate-900 px-1.5 pb-1 pt-1 shadow-inner"
          : "flex-1 rounded-2xl bg-slate-900 p-2 shadow-inner"
      }
    >
      {/* ── Top: Control Panel Bar — Mode | value with separator ── */}
      <View
        className={`flex-row items-center ${compact ? "pb-0.5" : "border-b border-slate-700 pb-1"}`}
      >
        ...
        >
          <Text
            numberOfLines={1}
            className={mono(
              `font-bold ${compact ? "text-[11px]" : "text-xs"} ${hl("mode") ? "text-slate-900" : "text-emerald-300"}`,
            )}
          >
            {shownName}
          </Text>
        ...
        <View
          className={
            compact
              ? "mx-1 h-3 w-px bg-slate-500"
              : "mx-1.5 h-3.5 w-px bg-slate-500"
          }
        />
        ...
        >
          <Text
            className={mono(
              `text-[10px] uppercase ${compact ? "" : "text-[10px]"} ${hl("speed") || hl("steer") ? "text-slate-900" : "text-slate-400"}`,
            )}
          >
            {rightLabel}
          </Text>
          <Text
            className={mono(
              `${compact ? "text-[11px]" : "text-xs"} font-bold ${hl("speed") || hl("steer") ? "text-slate-900" : "text-emerald-300"}`,
            )}
          >
            {" "}
            {rightFieldValue}
      ...
      <View className="min-h-0 flex-1 items-center justify-center">
        {body.kind === "dir" && (
          <Text
            numberOfLines={1}
            className={mono(
              `font-bold text-emerald-300 ${compact ? "text-[15px]" : "text-2xl"}`,
            )}
          >
            {body.text}
          </Text>
        ...
        {body.kind === "title" && (
          <Text
            numberOfLines={1}
            className={mono(
              `font-bold text-emerald-300 ${compact ? "text-[11px]" : "text-lg"}`,
            )}
          >
            {body.text}
          </Text>
        ...
        {body.kind === "coming" && (
          <Text
            numberOfLines={1}
            className={mono(
              `font-bold text-emerald-300 ${compact ? "text-[11px]" : "text-base"}`,
            )}
          >
            {body.text}
          </Text>
        ...
        {body.kind === "auto" && (
          <View className="w-full items-center">
            ...
            <View className="rounded-sm bg-slate-200 px-1.5 py-px">
              <Text
                className={mono(
                  `font-bold text-slate-900 ${compact ? "text-[11px]" : "text-sm"}`,
                )}
              >
                Angle:{" "}
            ...
            <View
              className={`w-full flex-row justify-between ${compact ? "px-1" : "px-2"}`}
            >
              <Text
                className={mono(
                  `text-emerald-300 ${compact ? "text-[11px]" : "text-xs"}`,
                )}
              >
                P:
                {telemetry.kp != null ? telemetry.kp.toFixed(2) : pidKpLocal()}
              </Text>
              <Text
                className={mono(
                  `text-emerald-300 ${compact ? "text-[11px]" : "text-xs"}`,
              ...
              >
                D:
                {telemetry.kd != null ? telemetry.kd.toFixed(3) : pidKdLocal()}
              </Text>
            </View>
            ...
            >
              ...
              >
                I:
                {telemetry.ki != null ? telemetry.ki.toFixed(3) : pidKiLocal()}
              </Text>
              <Text
                className={mono(
                  `text-emerald-300 ${compact ? "text-[11px]" : "text-xs"}`,
      ...
      <View
        className={`items-center rounded-sm bg-slate-200 ${compact ? "py-0.5" : "py-1"}`}
      >
        <Text
          numberOfLines={1}
          className={mono(
            `font-bold text-slate-900 ${compact ? "text-[11px]" : "text-xs"}`,
          )}
        >
          {compact
            ? (driveStatus || "READY").toUpperCase()
            : `${linkLabel} · ${bottomStatus}`}
        </Text>
      </View>
    </View>
  );
...
function pidKpLocal(): string {
  return "12.00";
}
function pidKiLocal(): string {
  return "3.000";
}
function pidKdLocal(): string {
  return "1.000";
}
function pidOutLocal(): string {
  return "0";
}
function pidOffLocal(): string {
  return "0.0";
}
