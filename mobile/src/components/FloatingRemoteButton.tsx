...
function anyLinked(): boolean {
  return sppService.isConnected || bleService.isConnected || wifiService.isConnected
}

function isRemoteInFront(): boolean {
  return navigationRef.isReady() && navigationRef.getCurrentRoute()?.name === 'RemoteControl'
}
...
async function persistSlot(slot: Slot): Promise<void> {
  try { await AsyncStorage.setItem(SLOT_KEY, JSON.stringify(slot)) } catch { /* best effort */ }
}
  ...
  const pan = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_evt, gesture) => {
        ...
        if (!movedRef.current && (Math.abs(gesture.dx) >= DRAG_THRESHOLD || Math.abs(gesture.dy) >= DRAG_THRESHOLD)) {
          movedRef.current = true
        }
        if (!movedRef.current) return
        const { width, height } = Dimensions.get('window')
        setSlot({
          x: clamp(dragStart.current.x + gesture.dx, EDGE, width - FAB_SIZE - EDGE),
          y: clamp(dragStart.current.y + gesture.dy, EDGE, height - FAB_SIZE - EDGE),
        })
      },
