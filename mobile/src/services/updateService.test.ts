...
const URL_323 =
  "https://storage.example.com/app-releases/genum-solutions-3.2.3.apk";
const URL_321 =
  "https://storage.example.com/app-releases/genum-solutions-3.2.1.apk";
...
describe("cacheFileNameFor (R-20a: per-release cache targets)", () => {
  ...
  it("flattens URL-unsafe characters for the filesystem", () => {
    expect(cacheFileNameFor("https://x.test/a/genum.apk?alt=media")).toBe(
      "genum.apk_alt_media",
    );
  });
...
describe("downloadAndInstall (R-20a cache-target behavior)", () => {
  ...
  it("surfaces a download-prefixed error and never opens the installer", async () => {
    ...
    await expect(downloadAndInstall(URL_323)).rejects.toThrow(
  ...
  it("surfaces an install-prefixed error when the installer cannot open", async () => {
    vi.mocked(IntentLauncher.startActivityAsync).mockRejectedValueOnce(
      new Error("no intent handler"),
    );

    await expect(downloadAndInstall(URL_323)).rejects.toThrow(
      /install:.*no intent handler/,
    );
  });
});
