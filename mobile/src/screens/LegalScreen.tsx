...
// website's app/privacy and app/terms pages). Legal entity details come
// from the shared company_info table (bundled copy as fallback).
// =====================================================================
...
function buildPrivacySections(company: Company): Section[] {
  return [
    {
      title: "Who we are",
      blocks: [
...
function buildTermsSections(company: Company): Section[] {
  return [
    {
      title: "Agreement",
      blocks: [
...
export function LegalScreen() {
  ...
  // Legal entity details come from the shared company_info table (bundled
  // copy as fallback until the read resolves) so app and website stay in sync.
  ...
  const sections = isPrivacy
    ? buildPrivacySections(company)
    : buildTermsSections(company);
            ...
            <View className="mt-3">
              {section.blocks.map((block) => (
                <View key={block} className="mb-3 flex-row items-start gap-2">
                  ...
                  <Text className="flex-1 text-sm leading-7 text-muted">
                    {block}
                  </Text>
                </View>
              ))}
