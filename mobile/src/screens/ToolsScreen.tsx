...
export function ToolsScreen() {
  ...
  const {
    ...
    canControl, sppSupported,
    sppDevices, scanning, connecting, handleScan, handleConnect,
    sppDevices: sppArr, scanning: scanning2, connecting: connecting2,
    // mode/category
    ...
    >
      ...
      <View className="mt-6">
        <View className="flex-row items-center justify-between">
          ...
          {sppStatus === 'connected' && (
            <Pressable onPress={() => navigation2.navigate('Tools' as never)} >
        ...
        <View className={`mt-4 rounded-2xl border border-line bg-card p-5 shadow-card ${!sppSupported ? 'opacity-50' : ''}`}>
          ...
          <View className={sppSupported ? '' : 'opacity-40'} pointerEvents={sppSupported ? 'auto' : 'none'}>
            ...
            />
            <Pressable
              onPress={wifiConnected ? () => navigation2.navigate('Tools' as never) : () => {}}
  ...
  return (
    ...
    >
      ...
      {/* Fullscreen remote-control window removed — handled by RemoteControlScreen */}
