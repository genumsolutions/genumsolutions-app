...
//
// Single mode: CAR REMOTE — per-product or generic robot-car remote with
// ModeChooser, connection card, PID/drive/balance controls, and mode info.
// Works with or without a productId route param.
// =====================================================================
...
export function ToolsScreen() {
  ...
  const {
    ...
    canControl, speed, servo, steerLimit, trim, driveStatus, telemetry,
  ...
  const displayMode = carMode ?? activeMode
  ...
  const isAuto = displayMode.controls.includes('pid-auto')
  const isAutonomous = displayMode.controls.includes('start-stop')
  const isWeblink = displayMode.controls.includes('weblink')
  const is2wd1m = displayMode.controls.includes('drive-2wd1m')
  const isRf = displayMode.transport.includes('rf')
  ...
  return (
    <ScrollView className="flex-1 bg-mist" contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      ...
      <Text className="text-xs font-black uppercase tracking-[0.24em] text-navy">
        Control Panel · {displayMode.token}
      </Text>
      <Text className="mt-1 font-display text-2xl font-bold text-ink">{carProduct?.name ?? 'Car'}</Text>
      <Text className="mt-1 text-xs leading-5 text-muted">{displayMode.blurb}</Text>
        ...
        <>
              ...
              {connTab === 'bluetooth' && (
                <View className="mt-4">
                  <Text className="text-xs leading-5 text-muted">
                    Scan and connect to your {displayMode.name.toLowerCase()} hardware. PIN: 1234.
                  </Text>
                  {!sppSupported && (
          ...
          {isAuto ? (
            <BalanceControls
              ...
              off={pidOff}
              onPid={applyPid}
              onEnterMode={() => selectMode(displayMode)}
            />
          ) : isAutonomous ? (
            <AutonomousControls
              canControl={canControl}
              ...
              onSpeed={handleSpeed}
              onRun={() => { sendCommand(displayMode.token); }}
              ...
              {canControl && !isWeblink && (
                <View className="mt-4 flex-row items-center justify-between gap-3 rounded-2xl border border-line bg-card px-4 py-3">
                  <Text className="min-w-0 flex-1 text-xs leading-4 text-muted">
                    If the car is not in {displayMode.token} mode, tap to switch.
                  </Text>
                  <Pressable onPress={() => selectMode(displayMode)} className="shrink-0 rounded-full bg-navy px-4 py-2">
                    <Text className="text-xs font-black text-white">Enter {displayMode.token}</Text>
                  </Pressable>
                </View>
              )}
              {canControl && !isWeblink && (
      ...
      <View className="mt-6 rounded-lg border border-line bg-card p-4">
        ...
        <Text className="mt-0.5 text-xs text-muted">
          App v{APP_VERSION} · Control Panel · {displayMode.token} ({displayMode.transport.join(', ')})
        </Text>
      </View>
