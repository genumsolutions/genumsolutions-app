  ...
  const handleInstall = useCallback(async () => {
    ...
    try {
      ...
      setState({
        status: "error",
        error:
          "Download or install failed. Check your connection and try again.",
        latestVersion: state.latestVersion,
      });
    }
  }, [state]);
          ...
          <View className="mt-4 rounded-lg border border-line bg-card p-4">
            ...
            {state.status === "checking" && (
              <View className="flex-row items-center">
                ...
                <Text className="ml-3 text-sm text-muted">
            ...
            {state.status === "up-to-date" && (
              <>
                ...
                <Text className="mt-1 text-sm leading-6 text-muted">
                  v{state.latestVersion} is the newest version. Come back later
            ...
            {state.status === "installing" && (
              <Text className="text-sm leading-6 text-muted">
                The installer is open. Tap INSTALL to update to v
                {state.latestVersion} — then reopen the app.
              </Text>
            )}
